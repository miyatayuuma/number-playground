...
try {
 ...
 µ¨¥Â¸­yêë¢°k¢G§¦*^ // until the currently active quantity has actually impacted.
  ...
  assert.doesNotMatch(visibleText, /base\s*[345]|[345]é€²|[0-9]+â‚?[345]/i);
  ...
  await route("gear", (p) => p.ammo[0] === 12 && p.ammo[1] === 20);
  console.log(
    "Native touch: slow 4, fast 4â†’2â†’1, fast whole grip, single-dot contact and cancelled peels passed.",
  );
  ...
  assert.match(await bp.locator("#stage-name").textContent(), /ã‚®ã‚¢/);
